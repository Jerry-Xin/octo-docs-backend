...
**变更类型**：新增 1 个 node。
**新增 Node**：
- `image`（block 或 inline，按前端 NodeView 定）— attrs: `{ attachId, src, alt, title, width, align }`（camelCase，与前端 Tiptap image 扩展的 attr 名字节对齐）；parseDOM/toDOM 用 `data-attach-id` ↔ `attachId`、`title` ↔ `title`；**红线：src 严禁 base64 入 Y.Doc,只存 attachId + presign 换发的 URL**（backend §3.5）
